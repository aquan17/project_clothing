                ...
                <div class="col-lg-8">
                    <div class="card shadow">
                        <div class="card-body p-4 p-md-5">
                            ...
                            <p class="mb-1"><strong>Địa chỉ:</strong> {{ $order->shippingAddress->province ?? 'N/A' }},
                                {{ $order->shippingAddress->district ?? 'N/A' }},
                                {{ $order->shippingAddress->ward ?? 'N/A' }},
                                {{ $order->shippingAddress->country ?? 'N/A' }}</p>
                            <p class="mb-1"><strong>Số điện thoại:</strong> {{ $order->shippingAddress->phone ?? 'N/A' }}
                            </p>
                                ...
                                <div class="d-flex justify-content-between align-items-center border-bottom py-2">
                                    <div>
                                        ...
                                        @if ($item->productVariant)
                                            <small>Phân loại: {{ $item->productVariant->color }} /
                            ...
                            <div class="mt-4 pt-2 hstack gap-2 justify-content-center">
                                <a href="{{ route('client.profile') }}" class="btn btn-primary btn-hover">Xem lịch sử đơn
                                    hàng <i class="ri-arrow-right-line align-bottom ms-1"></i></a>
                                <a href="{{ route('client.home') }}" class="btn btn-soft-danger btn-hover">Về trang chủ <i
