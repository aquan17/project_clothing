/* -------------------------------------------------------
 ...
 ------------------------------------------------------- */
//  Xử Lý Cart
// Cập nhật tổng số tiền khi có thay đổi checkbox
function updateTotal() {
    ...
    document.querySelectorAll('.product-checkbox').forEach(function (checkbox) {
        if (checkbox.checked) {
            ...
            const priceText = row.querySelector('[data-title="Price"]').innerText.replace(/[^\d]/g, '');
            const price = parseFloat(priceText);

            const quantityInput = row.querySelector('.qty-text');
...
function toggleSelectAll() {
    ...
    const selectAllText = document.getElementById('select-all');
    ...
    if (isChecked) {
        selectAllText.classList.remove('selected');
        selectAllText.innerHTML = 'Chọn tất cả';
    } else {
        selectAllText.classList.add('selected');
        selectAllText.innerHTML = 'Bỏ chọn tất';
    }
}

function attachCheckboxEvents() {
    document.querySelectorAll('.product-checkbox').forEach(cb => {
        cb.removeEventListener('change', updateTotal); // tránh gắn trùng
...
function changeQuantity(itemId, change) {
    ...
    fetch(`/cart/update/${itemId}`, {
        ...
        .then(data => {
            if (data.success) {
                qtyInput.value = data.quantity; // Đồng bộ quantity từ server

                // Cập nhật thành tiền từng sản phẩm
                const priceElement = qtyInput.closest('tr').querySelector('[data-title="Price"]');
                const subtotalElement = qtyInput.closest('tr').querySelector('[data-title="Total"]');
                ...
                // document.getElementById('cart-total').innerText = data.cartTotal.toLocaleString('vi-VN') + 'đ';
            } else {
            ...
            fetch(`/cart/remove/${itemId}`, {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        if (!document.querySelectorAll('tr[data-item-id]').length) {
                            document.getElementById('cart-items').innerHTML = '<tr><td colspan="7" class="text-center">No items in this order.</td></tr>';
                        }
                    } else {
...
$(document).ready(function () {
    $('#add-to-cart-form').on('submit', function (e) {
        ...
        if (!isLoggedIn) { // biến isLoggedIn ta sẽ set từ server
            alert('Bạn cần đăng nhập để thêm sản phẩm vào giỏ hàng.');
            window.location.href = '/login'; // Hoặc chỉ alert thôi nếu bạn không muốn tự chuyển
            return;
        }
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                // Bạn có thể cập nhật số lượng giỏ hàng ở đây
            },
            error: function (xhr) {
...
document.addEventListener('DOMContentLoaded', function () {
    const locations = {
        'Hà Nội': {
            'Cầu Giấy': ['Dịch Vọng Hậu', 'Mai Dịch', 'Trung Hòa'],
            'Ba Đình': ['Phúc Xá', 'Kim Mã', 'Ngọc Hà'],
        },
        'TP. Hồ Chí Minh': {
        ...
        'Đà Nẵng': {
            'Hải Châu': ['Thạch Thang', 'Hòa Cường Bắc', 'Hòa Thuận Tây'],
        },
    };
    ...
    const resultProvince = document.getElementById('selected-province');
    const resultDistrict = document.getElementById('selected-district');
    const resultWard = document.getElementById('selected-ward');

    // Tạo danh sách các tỉnh
    ...
    // Tạo danh sách các quận tương ứng với tỉnh
    ...
    // Tạo danh sách các xã/phường tương ứng với quận
    ...
    // Khi chọn quận, load xã/phường tương ứng
    ...
    // Khi chọn xã/phường, hiển thị kết quả
    wardSelect.addEventListener('change', function () {
        const province = provinceSelect.value;
        const district = districtSelect.value;
        const ward = this.value;

        resultProvince.textContent = `Tỉnh: ${province}`;
        resultDistrict.textContent = `Quận/Huyện: ${district}`;
        resultWard.textContent = `Xã/Phường: ${ward}`;
    });
